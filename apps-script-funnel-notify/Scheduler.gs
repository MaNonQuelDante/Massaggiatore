...
function checkFunnelNotifications() {
  ...
  var finestraFine   = new Date(nowMs + 5 * 60 * 1000);
...
function processaEventoFunnel_(ev, props, nowMs, dryRun) {
  ...
  // 🔄 STOP FUNNEL se "Confermato" nel foglio: niente più email per questo lead.
  if (FunnelStore.isConfirmed(telefono, leadKey)) {
    Logger.log('🔄 Lead confermato → salto evento: %s', ev.getTitle());
    return;
  }
  ...
  for (var k = 0; k < CONFIG.SOGLIE.length; k++) {
    ...
    for (var n = 0; n < NOTIFIERS.length; n++) {
      ...
      if (!lead) lead = buildLeadFromEvent_(ev, t0, telefono, rec);
...
function buildLeadFromEvent_(ev, t0, telefono, rec) {
...
function test() {
  ...
  for (var c = 0; c < cals.length; c++) {
    ...
    var calMatch = cal.getName().toLowerCase().indexOf(CONFIG.CAL_MATCH) !== -1;
    var eventi;
    try { eventi = cal.getEvents(finestraInizio, finestraFine); } catch (e) { continue; }
    for (var i = 0; i < eventi.length; i++) {
      var ev = eventi[i];
      if (ev.isAllDayEvent()) continue;
      var titolo = (ev.getTitle() || '').trim().toLowerCase();
